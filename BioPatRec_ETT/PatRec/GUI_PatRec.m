...
% Last Modified by GUIDE v2.5 05-Jun-2012 13:45:32
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    if ~isequal(file, 0)
        ...
        elseif (exist('patRec','var'))              % Get patRec
            Load_patRec(patRec, 'GUI_PatRec',0);
...
%    set(handles.pb_RunOfflineTraining,'Enable','Off');    
    ...
    %Normalize
    %normSets = get(handles.cb_normalize,'Value'); 
...
%    %Normalize
%    indvMov = get(handles.cb_indvMov,'Value');     
    ...
    %Algorithm selection
%    aIdx = get(handles.pm_SelectAlgorithm,'Value');
%    tIdx = get(handles.pm_SelectTraining,'Value');    
    ...
    patRec = OfflinePatRec(sigFeatures, selFeatures, randFeatures, normSets, alg, tType, movMix, topology, confMatFlag);
...
%    alg = get(hObject,'Value'); %returns pm_SelectAlgorithm contents as cell array
    ...
    elseif strcmp(alg,'MLP') || strcmp(alg,'MLP + RFN') % for ANN
...
%        Load_sigFeatures(sigFeatures, handles);
        ...
        for j = 1 : nRep
            ...
            set(handles.t_msg,'String',['Statistics Rep: ' num2str(j) ' from: ' num2str(i)]);
            drawnow;
        end        
    ...
    stats.nM        = nM;
    stats.features  = features;
    stats.algorithm = patRec.patRecTrained.algorithm;
    stats.training  = patRec.patRecTrained.training;
    stats.topology  = patRec.topology;
