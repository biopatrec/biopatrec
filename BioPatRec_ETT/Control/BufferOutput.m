...
function [patRec outMov] = BufferOutput(patRec,outMov)

% Remove the oldest prediction
patRec.outBuffer = patRec.outBuffer(2:end,:);
% Add the new prediction
patRec.outBuffer(end+1,outMov) = 1;

%% Compute output
% Not suitable for simltaneous control
outMov = find(0.5 <= sum(patRec.outBuffer)./size(patRec.outBuffer,1));
