...
function tac = VREActivation(vre_Com, movDist, movTime, movDof, movDir, moveTac)
    if ~moveTac
    ...
    tac = 0;
    fwrite(vre_Com,sprintf('%c%c%c%c',char(handMoved),char(movDof),char(movDir),char(movDist)));
    ack = fread(vre_Com,1);
    if ack == 't'
