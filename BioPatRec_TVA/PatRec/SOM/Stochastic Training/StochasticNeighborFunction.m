...
function neigh=StochasticNeighborFunction(SOM,sigma,i0)
...
switch neighborFunction,
    case 'bubb'
        ...
        neigh=(neuronDist(:,i0)<=sigma);
    case 'gauss'
        neigh = exp(-(neuronDist(:,i0).^2)./(2*sigma^2));
    case 'cutGauss'
        neigh = exp(-(neuronDist(:,i0).^2)./(2*sigma^2)) .* (neuronDist(:,i0)<=sigma);
    case 'trai'
        neigh = (1-neuronDist(:,i0)./sigma) .* (neuronDist(:,i0)<=sigma);
    case 'butter'
        neigh =1./(1+(neuronDist(:,i0)./sigma).^4);
end