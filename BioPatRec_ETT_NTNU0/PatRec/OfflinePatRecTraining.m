...
function patRec = OfflinePatRecTraining(alg, tType, algConf, trSets, trOuts, vSets, vOuts, mov, movIdx)
    ...
    elseif strcmp(alg,'Proportional Control')
        [PropCon, accV] = OfflineProportionalControl( tType, trSets, trOuts, vSets, vOuts, mov );
        patRec.algorithm = 'Proportional Control';
        patRec.training = tType;
        patRec.PropCon=PropCon;
        
%         % Load GUI
%         Load_patRec(handles.patRec, 'GUI_ProportionalControl',1);
%         GUI_ProportionalControl();
