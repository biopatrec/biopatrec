...
% 2013-05-21 / Alf Bjørnar Bertnum / Added an if-condition checking whether
%                                   Proportional Control is chosen; and if
%                                   so, it executes GUI_ProportionalControl
...
function Load_patRec(patRec, newGUI, loadMovements)
    ...
    % Check if chosen control algorithm is proportional control
    if strcmp(patRec.patRecTrained.algorithm,'Proportional Control')
        eval(['GUI_ProportionalControl','(patRec)']);
        return
    end
