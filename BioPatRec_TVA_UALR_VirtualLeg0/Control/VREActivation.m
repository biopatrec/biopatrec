...
function tac = VREActivation(vre_Com, movSpeed, movTime, movDof, movDir, moveTac)
    ...
    ack = fread(vre_Com,1)
    % The below fwrite is used to avoid a delay in the virtual reality
    % environment. Only for the virtual leg
    fwrite(vre_Com,sprintf('%c%c%c%c%c',1,0,0,0,0));
    ack1 = fread(vre_Com,1)
